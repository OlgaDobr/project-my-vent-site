	  ...
	  return (
	    <Container className="mt-4">
	      ...
	      {orders.length === 0 ? (
	        ...
	        orders.map(o => (
	          <Card className="mb-3" key={o.id}>
            <Card.Body>
	              <Card.Title>Заказ {o.id}</Card.Title>
	              <Card.Text>Дата: {new Date(o.date).toLocaleString()}</Card.Text>
	              <ListGroup variant="flush">
	                {o.items.map(it => (
	                  <ListGroup.Item key={it.id}>
	                    {it.name} - {it.qty} шт. x {it.price} ₽
	                  </ListGroup.Item>
	                ))}
	              </ListGroup>
	              <div className="mt-2 font-weight-bold">
	                Итого: {new Intl.NumberFormat('ru-RU', { style: 'currency', currency: 'RUB' }).format(o.total)}
	              </div>
	            </Card.Body>
	          </Card>
	        ))
